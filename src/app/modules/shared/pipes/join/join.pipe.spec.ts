...
describe("JoinPipe", () => {
  ...
  it("should throw an error: It should be Array", () => {
    expect(() => pipe.transform("1000")).toThrowError(
      "JoinPipe: Invalid values. It should be Array"
    );
  });
  it("should throw an error: not a string", () => {
    expect(() => pipe.transform(mockValues, 1000)).toThrowError(
      "JoinPipe: not a string"
    );
  });
});
